...
export default {
  ...
  methods: {
    ...
    loadJobs (starting = 0, refreshing = false) {
      ...
      this.$http.get(window.Horizon.basePath + '/api/jobs/failed?' + tagQuery + 'starting_at=' + starting)
        .then(response => {
          ...
          if (!this.$root.autoLoadsNewEntries && refreshing && this.jobs.length && window._.first(response.data.jobs).id !== window._.first(this.jobs).id) {
    ...
    retry (id) {
      ...
      this.$http.post(window.Horizon.basePath + '/api/jobs/retry/' + id)
        .then(() => {
          setTimeout(() => {
            this.retryingJobs = window._.reject(this.retryingJobs, job => job == id);
          }, 5000);
        });
    },
     ...
     */
    isRetrying (id) {
      return window._.includes(this.retryingJobs, id);
    },
     ...
     */
    hasCompleted (job) {
      return window._.find(job.retried_by, retry => retry.status == 'completed');
    },
