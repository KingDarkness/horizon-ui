...
export default {
  ...
  methods: {
    loadFailedJob (id) {
      ...
      this.$http.get(window.Horizon.basePath + '/api/jobs/failed/' + id)
        .then(response => {
     ...
     */
    reloadRetries () {
      this.$http.get(window.Horizon.basePath + '/api/jobs/failed/' + this.$route.params.jobId)
        .then(response => {
    ...
    retry (id) {
      ...
      this.$http.post(window.Horizon.basePath + '/api/jobs/retry/' + id)
        .then(() => {
          setTimeout(() => {
            this.reloadRetries();
    ...
    prettyPrintException (exception) {
      var lines = window._.split(exception, "\n"),
        output = '';
...
<template>
  <div>
    <div class="card">
      <div class="card-header d-flex align-items-center justify-content-between">
        ...
        <button
          ...
          v-on:click.prevent="retry(job.id)"
        >
          <svg
      ...
      <div
        class="card-body card-bg-secondary"
        v-if="ready"
      >
        <div class="row mb-2">
    ...
    <div
      class="card mt-4"
      v-if="ready && job.retried_by.length"
    >
      <div class="card-header d-flex align-items-center justify-content-between">
      ...
      <table class="table table-hover table-sm mb-0">
        ...
        <tbody>
          ...
          <tr
            v-for="retry in job.retried_by"
            :key="retry.id"
          >
            <td>
              <svg
                v-if="retry.status == 'completed'"
                class="fill-success"
                viewBox="0 0 20 20"
              ...
              <svg
                v-if="retry.status == 'reserved' || retry.status == 'pending'"
                class="fill-warning"
                viewBox="0 0 20 20"
                style="width: 1.5rem; height: 1.5rem;"
              >
            ...
            <td class="table-fit">
              <a
                v-if="retry.status == 'failed'"
                :href="window.Horizon.basePath + '/failed/'+retry.id"
              >
                {{ retry.id }}
              </a>
