...
export default {
  ...
  methods: {
    ...
    loadMetric () {
      ...
      this.$http.get(window.Horizon.basePath + '/api/metrics/' + this.$route.params.type + '/' + encodeURIComponent(this.$route.params.slug))
        .then(response => {
    ...
    prepareData (data) {
      return window._.chain(data)
        ...
        .map(value => {
          return window._.reduce(value, (sum, value) => {
            return {
    ...
    buildChartData (data, attribute, label) {
      return {
        labels: window._.map(data, 'time'),
        datasets: [
          {
            label: label,
            data: window._.map(data, attribute),
            lineTension: 0,
...
<template>
  <div>
    <div class="card">
      ...
      >
        <p
          class="text-center m-0 p-5"
          v-if="ready && !rawData.length"
        >
          Not Enough Data
        </p>
