...
export default {
  ...
  methods: {
    ...
    loadTags () {
      ...
      this.$http.get(window.Horizon.basePath + '/api/monitoring')
        .then(response => {
     ...
     */
    openNewTagModal () {
      window.$('#addTagModel').modal({
        backdrop: 'static',
      });

      window.$('#newTagInput').focus();
    },
    ...
    monitorNewTag () {
      ...
      this.$http.post(window.Horizon.basePath + '/api/monitoring', { 'tag': this.newTag })
        .then(() => {
          window.$('#addTagModal').modal('hide');

          this.tags.push({ tag: this.newTag, count: 0 });

          window.$('#addTagModel').modal('hide');
     ...
     */
    stopMonitoring (tag) {
      this.$http.delete(window.Horizon.basePath + '/api/monitoring/' + encodeURIComponent(tag))
        .then(() => {
          this.tags = window._.reject(this.tags, existing => existing.tag == tag)
        })
    }
  }
}
