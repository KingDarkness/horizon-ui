...
export default {
  ...
  methods: {
    ...
    loadJobs (tag, starting = 0, refreshing = false) {
      ...
      this.$http.get(window.Horizon.basePath + '/api/monitoring/' + encodeURIComponent(tag) + '?starting_at=' + starting + '&limit=' + this.perPage)
        .then(response => {
      ...
      <tbody>
        ...
        >
          ...
          >
            ...
            <svg
              v-if="job.status == 'reserved' || job.status == 'pending'"
              class="fill-warning"
              viewBox="0 0 20 20"
              style="width: 1.5rem; height: 1.5rem;"
            >
