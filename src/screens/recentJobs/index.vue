...
export default {
  ...
  methods: {
    ...
    loadJobs (starting = -1, refreshing = false) {
      ...
      this.$http.get(window.Horizon.basePath + '/api/jobs/recent?starting_at=' + starting + '&limit=' + this.perPage)
        .then(response => {
          if (!this.$root.autoLoadsNewEntries && refreshing && this.jobs.length && window.window._.first(response.data.jobs).id !== window.window._.first(this.jobs).id) {
