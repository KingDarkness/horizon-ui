<template>
  <div>
    <div class="card">
      ...
      <div
        class="card-body card-bg-secondary collapse show"
        id="collapseDetails"
        v-if="ready"
      >
        <div class="row mb-2">
        ...
        <div class="row">
          ...
          >{{readableTimestamp(job.completed_at)}}</div>
          <div
            class="col"
            else
          >-</div>
    ...
    >
      ...
      <div
        class="card-body code-bg text-white collapse show"
        id="collapseData"
      >
    ...
    <div
      class="card mt-4"
      v-if="ready && job.payload.tags.length"
    >
      <div class="card-header d-flex align-items-center justify-content-between">
        <h5>Tags</h5>
...
export default {
  ...
  methods: {
    ...
    loadJob (id) {
      ...
      this.$http.get(window.Horizon.basePath + '/api/jobs/recent/' + id)
        .then(response => {
